...
namespace Triangle.AbstractMachine.Interpreter {
	public class Interpreter {
		...
		private int Content(Register register) {
			// Returns the current content of register r, even if r is one of the pseudo-registers L1..L6.
			switch(register) {
				case Register.CB:
		...
		private void Dump() {
			...
			if(_stackTop == StackBase)
				...
				int dynamicLink = _localBase,
					staticLink = _localBase;
				Register localRegNum = Register.LB;
				Console.WriteLine("      ST--> |////////|");
				Console.WriteLine("            |--------|");
				for(int addr = _stackTop - 1; addr >= StackBase; addr--) {
		...
		private int ToInt(bool value) { return value ? Machine.TrueValue : Machine.FalseValue; }
		...
		private void CallPrimitive(int primitiveDisplacement) {
			...
			switch(primitive) {
				...
				case Primitive.GET:
					_stackTop--;
					addr = data[_stackTop];
					try { currentChar = Console.Read(); }
					catch(IOException) { _status = Status.FailedIoError; }
				...
				case Primitive.GETEOL:
					try { while((currentChar = Console.Read()) != '\n'); }
					catch(IOException) { _status = Status.FailedIoError; }
					break;
		...
		private void InterpretProgram() {
			...
			do {
				// Fetch instruction ...
				Instruction currentInstr = Machine.Code[_codePointer];
				// Decode instruction ...
				OpCode op = currentInstr.OpCode;
				Register register = currentInstr.Register;
				int length = currentInstr.Length;
				int operand = currentInstr.Operand;
				// Execute instruction ...
				switch(op) {
					case OpCode.LOAD:
						int addr = operand + Content(register);
						CheckSpace(length);
		...
		private int LoadObjectProgram(string objectName) {
			...
			try {
				using(FileStream objectStream = new FileStream(objectName, FileMode.Open)) {
					...
					while(true) {
						Instruction instr = Instruction.Read(objectStream);
						if(instr == null)
							break;
			...
			catch(FileNotFoundException fnfe) { Console.WriteLine("Error opening object file: " + fnfe); }
			catch(IOException ioe) { Console.WriteLine("Error reading object file: " + ioe); }
			return Machine.CodeBase;
		}

		// RUNNING
		/**
         * Runs the interpreter with the given command-line arguments. There is a single, optional argument that is the filename of the object code to be
         * interpreted. If omitted, this defaults to the file "obj.tam" in the current directory.
         * @param	args	the command-line arguments
         */
		public static void Main(string[] args) {
			Console.WriteLine("********** TAM Interpreter (.NET CORE) **********");

			string objectName = args.Length == 1 ? args[0] : "obj.tam";
